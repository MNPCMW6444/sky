...
const App = ({sizeScale = 25, onDataLoad, mapStyle = MAP_STYLE}: AppProps) => {
        ...
        fetch(DATA_URL)
            ...
            .then(resp => {
                if (resp && resp.states && timer.id !== null) {
                    ...
                    if (data) {
                        const dataAsObj: any = {};
                        sortedData.forEach((entry: any) => (dataAsObj[entry[DATA_INDEX.UNIQUE_ID]] = entry));
                        sortedData = data.map((entry: any) => dataAsObj[entry[DATA_INDEX.UNIQUE_ID]] || entry);
                    }
    ...
    return (
        ...
        >
            <Map reuseMaps mapLib={maplibregl as unknown as MapLib<MapboxMap>} mapStyle={mapStyle}
            />
        </DeckGL>
    );
}
